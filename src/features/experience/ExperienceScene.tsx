...
export default function ExperienceScene({ mode, uuid, short_code }: Props) {
  const { experience } = useExperience(
    { uuid, short_code },
    { revalidateOnFocus: false }
  )

  // const [expSceneState, setExpSceneState] = useAtom(expSceneAtom)
  ...
  return (
    ...
    >
      ...
      <group position={[0, -0.5, 0]}>
        <Select>
          {experience?.asset_transform_info?.map(content => (
            <AssetContentSceneObject
              key={content.instance_id}
              content={content}
              access_token={experience?.access_token ?? undefined}
            />
          ))}
        </Select>
        {/* <Select>
